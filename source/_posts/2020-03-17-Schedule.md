...
Qt 的元对象系统（Meta-object-system）提供了对象之间通信的信号和槽机制、运行时类型信息和动态属性。

元对象系统由以下三个基础构成：

1、QObject类是所有使用元对象系统的类的基类；

2、在一个类的private 部分声明 Q_OBJECT 宏，使得类可以使用元对象的特性。如动态属性、信号与槽；

3、MOC（Meta-Object Compiler）元对象编译器为每个 QObject 的子类提供必要的代码来实现元对象系统的特性。

构建项目时，MOC 工具读取C++ 源文件时，当它发现类的定义里有Q_Object 宏时，它就会为这个类生成另外一个包含有元对象支持代码的C++ 源文件，这个生成的源文件连同类的实现文件一起被编译和链接。
